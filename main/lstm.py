...
nb_epochs = 10          # Number of iterations in the training phase
...
timestep_dir = "results/lstm/timestep_%d" % timestep
...
if not os.path.exists(path_result):
    ...
    if len(glob(path_result + '*')) == 2:
      ...
      #  exit()
        ...
        for file in glob(path_result + '*'):
            os.remove(file)
#%%

csv_path = "/content/drive/My Drive/project/packet_analysis_2.csv"  # Adjust the path to your CSV file if necessary
...
df = pd.concat([df, pd.DataFrame([dic_results])], ignore_index=True)
