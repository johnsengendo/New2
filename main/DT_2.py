...
h1.cmd("tc qdisc add dev h1-eth0 root tbf rate 100Mbit burst 15k latency 1ms")
s1.cmd("tc qdisc add dev s1-eth1 root tbf rate 100Mbit burst 15k latency 1ms")
s1.cmd("tc qdisc add dev s1-eth2 root tbf rate 100Mbit burst 15k latency 1ms")
s2.cmd("tc qdisc add dev s2-eth1 root tbf rate 100Mbit burst 15k latency 1ms")
s2.cmd("tc qdisc add dev s2-eth2 root tbf rate 100Mbit burst 15k latency 1ms")
h2.cmd("tc qdisc add dev h2-eth0 root tbf rate 100Mbit burst 15k latency 1ms")
...
# Function to calculate bandwidth for desired packets per second
def calculate_bandwidth(pps, packet_size):
    return pps * packet_size * 8  # bits per second
...
for index, row in data.iterrows():
    predicted_pps = row['Predicted_5']
    true_pps = row['True_5']
    
    # Using the true PPS value for the iperf command
    pps = true_pps
    
    # Calculate the required bandwidth
    bandwidth_bps = calculate_bandwidth(pps, packet_size)
    
    # Convert bandwidth to Mbps
    bandwidth_mbps = bandwidth_bps / 1e6
    
    # Run the iperf command
    iperf_command = f"iperf3 -c 10.0.0.2 -u -b {bandwidth_mbps}M -l {packet_size} -t 1 -i 1"
    h1.cmd(iperf_command)
    
    # Wait for 1 second before the next command
    time.sleep(1)
